...
def gold_turbine_data():
    turbine_sensor = dlt.read("silver_agg_turbine_data").withColumn("hour", F.date_format(F.col("window"), 'H:mm'))
    maintenance = dlt.read("silver_maintenance_data")
    power_output = dlt.read("silver_poweroutput_data")
    weather = dlt.read("silver_agg_weather_data")
