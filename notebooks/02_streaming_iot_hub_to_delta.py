...
# spark.sql(f"DROP TABLE IF EXISTS {target_database}.turbine_sensor_raw")
spark.sql(f"CREATE TABLE IF NOT EXISTS {target_database}.turbine_sensor_raw USING DELTA LOCATION '{BRONZE_PATH}turbine_sensor_raw'")
# spark.sql(f"DROP TABLE IF EXISTS {target_database}.weather_raw")
spark.sql(f"CREATE TABLE IF NOT EXISTS {target_database}.weather_raw USING DELTA LOCATION '{BRONZE_PATH}weather_raw'")
...
while True:
  try:
    spark.sql(f'CREATE TABLE IF NOT EXISTS {target_database}.turbine_sensor_agg USING DELTA LOCATION "{SILVER_PATH + "turbine_sensor_agg"}"')
    spark.sql(f'CREATE TABLE IF NOT EXISTS {target_database}.weather_agg USING DELTA LOCATION "{SILVER_PATH + "weather_agg"}"')
    break
