...
autoload = (
  ...
  .outputMode("append")
  .trigger(once=True)
  .option("checkpointLocation", f"{checkpoints_root_path}/maintenanceheader/")
  .option("mergeSchema", "true")
  .start(f"{delta_tables_root_path}/silver/maintenanceheader")
)
...
autoload = (
  ...
  .outputMode("append") 
  .trigger(once=True) 
  .option("checkpointLocation", f"{checkpoints_root_path}/poweroutput/") 
  .option("mergeSchema", "true")
  .start(f"{delta_tables_root_path}/silver/poweroutput")
)
...
spark.sql(f"""
              CREATE TABLE IF NOT EXISTS {target_database}.maintenance_header
              USING DELTA
              LOCATION '{delta_tables_root_path}/silver/maintenanceheader'
              """
          )
...
spark.sql(f"""
              CREATE TABLE IF NOT EXISTS {target_database}.power_output
              USING DELTA
              LOCATION '{delta_tables_root_path}/silver/poweroutput'
              """
          )